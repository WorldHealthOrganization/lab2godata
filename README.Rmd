---
title: "lab2godata: essential tools for matching lab results <img src='inst/images/godataR_logo.png' align='centre' height='18%' width='18%'/>"
...
The primary function that performs the matching is called `match_cases()`.  It has a few additional options compared to the Shiny app, so may be useful to run from an R environment for more experienced users.  The function uses two main methods to perform matching; 
  ...
  + a. First names and surnames are converted to soundex codes and then compared;
  + b. Age, dates of birth and identity card numbers are compared with the Damerau-Levenshtein method, allowing for a single transposition (except for age).
  + c. Exact matching (with no deviations allowed) is also available.
...
This package is hosted on the WHO Github Repository here: [https://github.com/WorldHealthOrganization/lab2godata](https://github.com/WorldHealthOrganization/lab2godata).
...
Bug reports and feature requests should be posted on github under [_issues_](https://github.com/WorldHealthOrganization/godataR/issues). All other questions and feedback, feel free to email us at [godata@who.int](mailto:godata@who.int) or post a query in the 
